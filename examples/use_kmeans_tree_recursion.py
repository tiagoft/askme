"""
Example usage of KMeansTreeRecursion class.

This script demonstrates how to use KMeansTreeRecursion to create a recursive tree
using K-means clustering from a collection of text documents.
"""
...
def main():
    """
    Example of using KMeansTreeRecursion.
    
    KMeansTreeRecursion automatically builds a complete tree by recursively applying
    KMeansTreeBuilder with stopping criteria.
    """
    
    # Step 1: Initialize the KMeansTreeBuilder
    ...
    # Step 2: Initialize KMeansTreeRecursion with stopping criteria
    ...
    recursion = KMeansTreeRecursion(
        ...
        min_split_ratio=0.05,  # Split should have at least 5% in smaller child
        max_split_ratio=0.95,  # Split should have at most 95% in smaller child
        max_depth=3,          # Maximum tree depth
    )
    print("KMeansTreeRecursion initialized successfully!")
    
    # Step 3: Build the recursive tree
    print("\nBuilding recursive K-means tree...")
    tree_root, global_metrics = recursion(text_collection)
    
    # Step 4: Display the tree structure
    print("\n=== Recursive K-means Tree Structure ===")
    ...
    with open('kmeans_recursive_tree.json', 'w') as f:
        f.write(json_string)
    print("Tree saved to kmeans_recursive_tree.json")
